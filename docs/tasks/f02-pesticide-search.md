...
FAMIC CSV から取り込んだ農薬マスターデータを PostgreSQL に格納し、作物名で農薬を検索して一覧表示する機能を実装する。農薬マスターデータは検索性（部分一致・JOIN）を重視し、Firestore ではなく PostgreSQL を使用する。
...
3. 基本部の各行を `pesticides` テーブルに UPSERT する
4. 適用部1 + 適用部2 の各行を `pesticide_applications` テーブルに INSERT する（取込前に既存データを DELETE → INSERT、またはトランザクション内で TRUNCATE → INSERT）
5. トランザクションで一括処理し、途中失敗時はロールバックする
...
**処理フロー:**
1. 入力された作物名で `pesticide_applications` テーブルを Prisma 経由で検索（完全一致）
2. リレーションを使って農薬基本情報を含めて取得
3. 農薬ごとに適用情報をグルーピングして返却
...
- `npx prisma migrate deploy` でテーブルが作成されること
- `importPesticides` を手動実行し、PostgreSQL にデータが投入されること
